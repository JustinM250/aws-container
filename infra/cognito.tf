...
# __generated__ by Terraform from "us-east-1_uWfXDQ5gw/6c7r5l0rk1jk70cvs4a657agdj"
resource "aws_cognito_user_pool_client" "the_cognito_user_pool_client" {
  ...
  name                                          = "Justin_McIntomney_XPIX"
  prevent_user_existence_errors                 = "ENABLED"
  read_attributes                               = []
  refresh_token_validity                        = 5
  region                                        = "us-east-1"
  supported_identity_providers                  = ["COGNITO"]
  user_pool_id                                  = aws_cognito_user_pool.the_cognito_user_pool.id // "us-east-1_uWfXDQ5gw" // REPLACE WITH REF
  write_attributes                              = []
...
# __generated__ by Terraform from "us-east-1_uWfXDQ5gw"
resource "aws_cognito_user_pool" "the_cognito_user_pool" {
  alias_attributes           = null
  auto_verified_attributes   = ["email"]
  deletion_protection        = "ACTIVE"
  mfa_configuration          = "OFF"
  name                       = "User pool - ggckk2"
  region                     = "us-east-1"
